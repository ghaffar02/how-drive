...
export default function Drivers() {
  ...
  const filteredDrivers = sampleDrivers.filter((driver) => {
    const matchesSearch =
      ...
      driver.number.toLowerCase().includes(searchQuery.toLowerCase());

    const matchesFilters =
      (filters.class.length === 0 || filters.class.includes(driver.class)) &&
      (filters.school.length === 0 || filters.school.includes(driver.school)) &&
      (filters.status.length === 0 || filters.status.includes(driver.status)) &&
      (filters.signup.length === 0 || filters.signup.includes(driver.signup));
  ...
  const handleOpenDialog = (driver: Driver) => {
    ...
    const classes = driver.class.split(',');
  ...
  const uniqueClasses = Array.from(new Set(sampleDrivers.map((d) => d.class)));
  const uniqueSchools = Array.from(new Set(sampleDrivers.map((d) => d.school)));
  const uniqueSignups = Array.from(new Set(sampleDrivers.map((d) => d.signup)));
                ...
                >
                  ...
                  >
                    <Typography
                      sx={{
                        color:
                          driver.status === 'Active' ? '#10b981' : '#ef4444',
                        fontWeight: 500,
                        fontSize: '14px',
                        fontFamily: '"Inter", sans-serif !important'
                      }}
              ...
              >
                ...
                <Box sx={{flex: '1 1 calc(25% - 18px)', minWidth: '200px'}}>
                  <TextField
                    ...
                    fullWidth
                    defaultValue={selectedDriver.school}
                    sx={{
                      '& .MuiInputBase-root': {
                        background: '#ffffff',
                        borderRadius: '8px'
        ...
        >
          ...
          <Box sx={{display: 'flex', gap: 2, justifyContent: 'center'}}>
            ...
            <Button
              ...
              onClick={() => {
                if (selectedDriver) {
                  const driverIndex = sampleDrivers.findIndex(
                    (d) => d.number === selectedDriver.number
                  );
                  if (driverIndex !== -1) {
                    handleDelete(driverIndex);
                  }
                }
              }}
              sx={{
